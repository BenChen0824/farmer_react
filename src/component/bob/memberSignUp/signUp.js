...
function SignUp(){
    ...
    const checkForm = async (event)=>{
        ...
        const email_re = new RegExp('^[a-zA-Z0-9._:$!%-]+@[a-zA-Z0-9.-]+.[a-zA-Z]$');
        ...
        if (data.checkPassword.length < 1 || data.password !== data.checkPassword) {
    ...
    <>
        <div className='bosu-bodyvh d-flex justify-content-center align-items-center bosu-bodybg'>
            <div className='shadow mb-5 bg-body rounded rounded-3 bg-white'>
                ...
                <form className="px-5 pb-4 pt-3 mx-2 needs-validation" noValidate name="form1" method="post" onSubmit={checkForm}>
                    <label htmlFor="username" className="sr-only m-2">姓名</label>
                        <input type="text" id="username" name="username" required className="form-control p-2 bg-light" placeholder="請輸入您的姓名"/>
                    <div className="invalid-feedback"></div>
                    <label htmlFor="email" className="sr-only m-2">E-mail</label>
                        <input type="email" id="email" name="email" required className="form-control p-2 bg-light" placeholder="請輸入您的E-mail"/>
                    <div className="invalid-feedback"></div>
                    <label htmlFor="password" className="sr-only m-2">密碼</label>
                        <input type="password" id="password" name="password" required className="form-control p-2 bg-light" placeholder="請輸入您的密碼"/>
                    <div className="invalid-feedback"></div>
                    <label htmlFor="checkPassword" className="sr-only m-2">確認密碼</label>
                        <input type="password" id="checkPassword" name="checkPassword" required className="form-control p-2 bg-light" placeholder="再次確認密碼"/>
                    <div className="invalid-feedback"></div>
                    ...
                    <div className="d-grid gap-2 my-4">
                        <button 
                        className="btn btn btn-dark btn-block" type="button" onClick={(event)=>{
                            event.preventDefault()
                            navigate('/member', {
                            replace: true,
                        })}}>返回登入頁面</button>
