        ...
        <div className='vh-100 d-flex justify-content-center align-items-center bosi-bodybg'>
            <div className='shadow mb-5 bg-body rounded rounded-3 bg-white'>
                ...
                <div className="bg-light p-3 rounded-top">
                        <h4 className="fw-semibold text-center m-0">會員登入</h4>
                </div>
                <form className="form-signin px-5 pb-4 pt-3 mx-2" name="form1" method="post" onSubmit={checkForm}>
                    ...
                    <input type="email" id="email" name="email" className="form-control p-2 bg-light" placeholder="會員帳號"/>
                    <label htmlFor="password" className="sr-only m-2">密碼</label>
                    <input type="password" id="password" name="password" className="form-control p-2 bg-light" placeholder="會員密碼"/>
                    <div className="checkbox m-3 text-end">
                        <label>
                        <a className="link-secondary text-decoration-none" href="#/">忘記密碼?</a>
                        </label>
                    </div>
                    <div className="d-grid gap-2">
                        <button 
                        className="btn btn btn-info btn-block" type="submit">登入</button> 
                        <button className="btn btn btn-dark btn-block" type="button" onClick={()=>{
                            navigate('/member/signup', {replace:true})
                        }}>立即加入會員</button>
                    </div>
                    <p className='m-3 text-center'>——— 其他方式登入 ———</p>
                    <div className="d-grid gap-2 col-10 mx-auto mb-2">
                        <button className="btn btn btn-success btn-block" onClick={LineAuth}><i className="fa-brands fa-line"></i>使用 LINE 帳號登入</button>
                    </div>
                </form>
            </div>
        </div>
